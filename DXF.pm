package App::Test::DWG::LibreDWG::DXF;
...
sub run {
	...
	my $dxf_file_first = catfile($tmp_dir, 'first.dxf');
	my $dwg_to_dxf_first = "$DR $v -o $dxf_file_first $dwg_file_first";
	if ($self->_exec($dwg_to_dxf_first, 'dwg_to_dxf')) {
		return 1;
	}

	# Convert JSON to dwg file.
	my $dwg_file_second = catfile($tmp_dir, 'second.dwg');
	my $dxf_to_dwg_first = "$DW $v -o $dwg_file_second $dxf_file_first";
	if ($self->_exec($dxf_to_dwg_first, 'dxf_to_dwg')) {
		return 1;
	}

	# Convert new dwg file to DXF.
	my $dxf_file_second = catfile($tmp_dir, 'second.dxf');
	my $dwg_to_dxf_second = "$DR $v -o $dxf_file_second $dwg_file_second";
	if ($self->_exec($dwg_to_dxf_second, 'dwg_to_dxf_second')) {
		return 1;
	}

	# Compare DXF files.
	my $diff = "diff $dxf_file_first $dxf_file_second";
	system($diff);
...
App::Test::DWG::LibreDWG::DXF - Base class for test-dwg-libredwg-dxf script.

=head1 SYNOPSIS

 use App::Test::DWG::LibreDWG::DXF;

 my $app = App::Test::DWG::LibreDWG::DXF->new;
